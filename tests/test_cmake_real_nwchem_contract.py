...
CAPNP_PARSER_FIXTURES = [
    ("nwchem_parser_params_bin", "NWCHEMC_PARSER_PARAMS_BIN"),
    ("nwchem_pspspin_params_bin", "NWCHEMC_PSPSPIN_PARAMS_BIN"),
    ("nwchem_nwpw_spin_mode_params_bin", "NWCHEMC_NWPW_SPIN_MODE_PARAMS_BIN"),
    (
        "nwchem_nwpw_allow_translation_params_bin",
...
class CMakeRealNWChemContractTest(unittest.TestCase):
    ...
    def test_capnp_parser_receives_all_parser_fixture_arguments(self):
        ...
        command = self._block(r"add_test\(\s*NAME\s+capnp-parser\b")

        self.assert_contains_all(
            dependencies, [target for target, _variable in CAPNP_PARSER_FIXTURES]
        )
        self.assert_contains_all(
            command, [variable for _target, variable in CAPNP_PARSER_FIXTURES]
        )
